...
from ...input_output import load_performance_regression
from ...input_output import save_performance_regression
...
def modelling(model_name, x_train, x_test, y_train, y_test, fraction=1, export=False, leaf_size=30, n_neighbors=5,
    ...
    if model_name != 'Mean':
        ...
        if model_name == 'Neural_Network_1hl' or model_name == 'Neural_Network_2hl': y_predict = list(
            itertools.chain.from_iterable(y_predict))  # Transform double list to single list
    else:
        y_predict = model
    performance = load_performance_regression()  # Load all performance statistics
    save_performance_regression(performance, model_name, y_test, y_predict, lapsed_time, fraction)  # Update performance statistics
...
def model_selection(name, x_train, y_train, y_test, leaf_size, n_neighbors, n_estimators, min_samples_split,
    ...
    if name == "Mean":
        ...
        df['duration'] = y_train.mean()
        return df['duration']
    elif name == 'OLS_Regression':
        return sklearn.linear_model.LinearRegression().fit(x_train, y_train)
    elif name == 'Random_Forest_Regression':
        return sklearn.ensemble.RandomForestRegressor(n_estimators=n_estimators,
                                                      min_samples_split=min_samples_split,
                                                      min_samples_leaf=min_samples_leaf).fit(x_train, y_train)
    elif name == 'KNN_Regression':
        return KNeighborsRegressor(leaf_size=leaf_size, n_neighbors=n_neighbors).fit(x_train, y_train)
...
def neural_network(hidden_layers, layers, batch_size, x_train, y_train, verbose=0):
    ...
    path = os.path.join(path_a, r'data\output\best_model.h5')
    model = Sequential()
